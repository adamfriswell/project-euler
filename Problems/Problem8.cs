using System;
using System.Collections.Generic;
using System.Linq;

namespace project_euler.Problems {
    public class Problem8 : ISolveable {

        public void Solve () {

            string input = "";
            List<char> charList = input.ToCharArray ().ToList ();

            List<int> digits = new List<int> ();
            foreach (var c in charList) {
                digits.Add (Int32.Parse (c.ToString ()));
            }

            List<Problem8Info> info = new List<Problem8Info> ();

            int count = 0;
            int numberOfAdjacentDigits = 13;

            while (count <= (digits.Count - numberOfAdjacentDigits)) {

                List<int> adjacentDigits = new List<int> ();
                long product = 1;
                int i = count;

                while (i <= count + numberOfAdjacentDigits - 1) {
                    adjacentDigits.Add (digits[i]);
                    product *= digits[i];
                    i++;
                }

                info.Add (new Problem8Info {
                    Count = count,
                    AdjacentDigits = adjacentDigits,
                    Product = product
                });

                count++;
            }

            Problem8Info result = new Problem8Info () { AdjacentDigits = new List<int> () { }, Product = 0 };

            foreach (var item in info) {
                if (item.Product > result.Product) {
                    result = item;
                }
            }

            string multiplication = "";
            foreach (var d in result.AdjacentDigits) {
                multiplication += d + " * ";
            }
            multiplication = multiplication.Substring (0, multiplication.Length - 3);

            Console.WriteLine ($"Problem 8: {result.Product} = {multiplication} (starting at {result.Count}th digit)");
        }
    }

    public class Problem8Info {
        public int Count { get; set; }
        public List<int> AdjacentDigits { get; set; }
        public long Product { get; set; }
    }
}